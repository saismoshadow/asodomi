...
// Testo semplice → paragrafi HTML (stessa logica del blog)
if ($descrizione !== '' && !preg_match('/<[a-z][^>]*>/i', $descrizione)) {
    $paragrafi = preg_split('/\n{2,}/', $descrizione) ?: [$descrizione];
    $descrizione = '<p>' . implode('</p><p>', array_map(
        fn($p) => str_replace("\n", '<br>', trim($p)),
        array_filter($paragrafi, 'strlen')
    )) . '</p>';
}
$descrizione = sanitizza_html($descrizione);
